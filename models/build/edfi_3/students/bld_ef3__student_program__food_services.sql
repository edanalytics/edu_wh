...
{% set exclude_programs = var('edu:food_service:exclude_programs') %}

{# customizable: the column that defines the start date for the homeless program #}
{% set start_date_column = var('edu:food_service:start_date_column') %}

{# customizable: the column that defines the end date for the homeless program #}
{% set exit_date_column = var('edu:food_service:exit_date_column') %}

{# customizable: extra indicators to create in the aggregate query #}
{% set custom_program_agg_indicators = var('edu:food_service:custom_program_agg_indicators', None) %}
...
maxed as (
    ...
    select 
        ...
        ) as is_food_service_active, -- if the student has an active food_service program enrollment

        max(
          {{ value_not_in_list(field='program_name', excluded_items=exclude_programs) }}
        ) as is_food_service_annual, -- the student had a food_service program enrollment any time during the year
...
xyear_agged as (
    select
        k_student_xyear,
        max(is_food_service_annual) as is_food_service_ever
...
joined as (
    select
        maxed.*,
        xyear_agged.is_food_service_ever
